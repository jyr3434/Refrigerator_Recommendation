...
def PageCrawler(urllist):
    ...
    for n in res.find_all('div', 'view_step_cont'):
        i = i + 1
        rec_step.append('#' + str(i) + ' ' + n.get_text().replace('\n', ' '))
...
def Getrcp(page_idx):
    ...
    i = 0
    #for page_idx in range(1, 2): # 만개의 레시피 총 3455페이지
...
if __name__=='__main__':
    ...
    reclist = pool.map(Getrcp, range(1,4))
    #reclist = pool.map(Getrcp, range(1, 40))  # get_contetn
    #pool.map(Getrcp(), '') # get_contetn 함수를 넣어줍시다.
    print(sum([len(i) for i in reclist]))
